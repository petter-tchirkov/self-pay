      ...
      <div class="item__info">
        ...
        <div class="item__acts">
          ...
          <div class="item__count">
            <button
              class="item__counter"
              @click="count >= 1 ? count-- : undefined"
            >
              -
            </button>
            <span>{{ count }}</span>
            <button class="item__counter" @click="count++">+</button>
          </div>
        </div>
      </div>
      <img class="item__close" src="/images/icons/close.svg" alt="" />
    </div>
  </div>
</template>
...
.item {
  ...
  flex: 1;
  transition: 0.3s;
  background: linear-gradient(180deg, #f4f4f4 0%, #f6f6f6 0.01%, #dddddd 100%);
