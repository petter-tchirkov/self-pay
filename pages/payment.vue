  ...
  <div class="wrapper">
    <section class="payment">
      <header class="payment__header">
        ...
        <h2 class="payment__title">{{ $t('payForOrder') }}?</h2>
      </header>
      <h2 class="payment__method">{{ $t('paymentMethod') }}</h2>
      <div class="payment__options">
        <label class="payment__option">
          <input v-model="selectedPaymentMethod" type="radio" value="mc" name="tip" />
          <img src="/images/icons/payMethods/mc.png" alt="" />
        </label>
        <label class="payment__option">
          <input v-model="selectedPaymentMethod" type="radio" value="visa" name="tip" />
          <img src="/images/icons/payMethods/visa.png" alt="" />
        </label>
        <label class="payment__option">
          <input v-model="selectedPaymentMethod" type="radio" value="cash" name="tip" />
          <span>
            {{ $t('cash') }}
          </span>
        </label>
        <label class="payment__option">
          <input v-model="selectedPaymentMethod" type="radio" value="ap" name="tip" />
          <img src="/images/icons/payMethods/ap.png" alt="" />
        </label>
        <label class="payment__option">
          <input v-model="selectedPaymentMethod" type="radio" value="gp" name="tip" />
          <img src="/images/icons/payMethods/gp.png" alt="" />
        </label>
        <label class="payment__option">
          <input v-model="selectedPaymentMethod" type="radio" value="pp" name="tip" />
          <img src="/images/icons/payMethods/pp.png" alt="" />
        </label>
      </div>
      <div class="payment__card">
        <h3 class="payment__part">{{ $t('cardNumber') }}</h3>
        <InputMask :disabled="selectedPaymentMethod !== 'mc' && selectedPaymentMethod !== 'visa'
          " class="input-shadow" mask="9999 9999 9999 9999" placeholder="XXXX XXXX XXXX XXXX" />
      </div>
      <div class="flex justify-between gap-6">
        <div class="payment__card">
          <h3 class="payment__part">{{ $t('expirationDate') }}</h3>
          <InputMask :disabled="selectedPaymentMethod !== 'mc' && selectedPaymentMethod !== 'visa'
          " class="input-shadow" mask="99/99" placeholder="ММ/РР" slotChar="ММ/РР" />
        </div>
        <div class="payment__card">
          <h3 class="payment__part">{{ $t('cvv') }}</h3>
          <input :disabled="selectedPaymentMethod !== 'mc' && selectedPaymentMethod !== 'visa'
          " type="password" maxlength="3"
            class="input-shadow placeholder:text-[#060F0A] placeholder:text-3xl placeholder:tracking-widest" mask="999"
            placeholder="&middot;&middot;&middot;" slotChar="&middot;&middot;&middot;" />
        </div>
      </div>
      <div class="payment__card">
        <h3 class="payment__part">{{ $t('cardHolderName') }}</h3>
        <InputText class="input-shadow" :disabled="selectedPaymentMethod !== 'mc' && selectedPaymentMethod !== 'visa'
          " />
      </div>
      ...
      <div class="order__cost cost">
        ...
        <div class="flex justify-center">
          <ui-base-button @click="$router.push('/success')" :label="$t('orderPay')" is-green />
        </div>
      </div>
    </section>
  </div>
