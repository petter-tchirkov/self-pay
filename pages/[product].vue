<template>
  <section class="product">
    ...
    <div class="product__picture h-full w-full bg-cover grow bg-center"
    ...
    <div class="product__content">
      ...
      <div class="product__actions">
        ...
        <button class="product__add" @click="[addToOrder(dish), $router.back()]">
          {{ $t('addToOrder') }}
        </button>
      </div>
    </div>
...
const { fakeDishes } = storeToRefs(useDishesStore())

// const { data: dish } = await useFetch<Dish>(`${url}/Products/${product}`)

const dish = computed(() => {
  return fakeDishes.value.find((dish) => dish.productId === +product)
})
</script>
